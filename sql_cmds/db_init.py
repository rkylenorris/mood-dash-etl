...
def create_tables(db_conn=create_db_conn(str(db_path))):
    ...
    rolling_calendar.to_sql('calendar', db_conn,
...
def insert_prefs(prefs_dict, db_conn=create_db_conn(str(db_path))):
    ...
    VALUES (?, ?, ?, ?)
    '''
    bckup = next(filter(lambda x: x['key'] ==
                 'AUTO_BACKUP_IS_ON', prefs_dict))['value']
    last_days_inarow = next(
        filter(lambda x: x['key'] == 'LAST_DAYS_IN_ROWS_NUMBER', prefs_dict))['value']
    longest_days_inarow = next(
        filter(lambda x: x['key'] == 'DAYS_IN_ROW_LONGEST_CHAIN', prefs_dict))['value']
    last_entry_time = next(
        filter(lambda x: x['key'] == 'LAST_ENTRY_CREATION_TIME', prefs_dict))['value']
