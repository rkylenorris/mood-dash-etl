...
def find_backup_file(pickup_dir: Path = PICKUP_DIR) -> Path | None:
    ...
    if not pickup_path.exists():
        ...
        return get_latest_backup(pickup_dir)
...
def get_selected_tables(selected_tables_path: Path = SELECTED_TABLES_PATH):
...
if __name__ == "__main__":
    pickup_path = find_backup_file()
    if pickup_path:
        print(f"Using backup file: {pickup_path.name}")
